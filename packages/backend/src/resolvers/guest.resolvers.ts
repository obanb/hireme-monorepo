    ...
    reservations: async (parent: { email: string }) => {
      ...
      try {
        ...
        return result.rows.map((row: Record<string, unknown>) => formatReservation({
          ...
          totalPrice: row.total_price ? parseFloat(row.total_price as string) : null,
          payedPrice: row.payed_price ? parseFloat(row.payed_price as string) : 0,
          currency: row.currency as string | null,
          roomIds: (row.room_ids as string[]) || [],
          accountId: row.account_id ? parseInt(row.account_id as string, 10) : null,
