...
interface BookingEnginePromoCodeDoc extends Document {
  data: string;
  created: Date;
...
const bookingEnginePromoCodeSchema = new Schema<BookingEnginePromoCodeDoc>(
  {
  ...
  {
    collection: "bookingEnginePromoCodes",
    timestamps: false,
  }
);

bookingEnginePromoCodeSchema.index({ originId: 1 });
bookingEnginePromoCodeSchema.index({ notificationId: 1 });
bookingEnginePromoCodeSchema.index({ reservationId: 1 });
bookingEnginePromoCodeSchema.index({ hotelId: 1, created: -1 });

export const BookingEnginePromoCodeModel: Model<BookingEnginePromoCodeDoc> =
  mongoose.models.BookingEnginePromoCode ??
  mongoose.model<BookingEnginePromoCodeDoc>("BookingEnginePromoCode", bookingEnginePromoCodeSchema);

// ── Mapper ────────────────────────────────────────────────────────────────────

export function toBookingEnginePromoCode(doc: BookingEnginePromoCodeDoc): BookingEnginePromoCode {
  return {
