...
interface BookingEngineNoteDoc extends Document {
  data: string;
  created: Date;
...
export interface BookingEngineNote {
  id: string;
  data: string;
  created: Date;
  dateOfUsage: Date | null;
...
const bookingEngineNoteSchema = new Schema<BookingEngineNoteDoc>(
  {
  ...
  {
    collection: "bookingEngineNotes",
    timestamps: false,
  }
);

bookingEngineNoteSchema.index({ originId: 1 });
bookingEngineNoteSchema.index({ notificationId: 1 });
bookingEngineNoteSchema.index({ reservationId: 1 });
bookingEngineNoteSchema.index({ hotelId: 1, created: -1 });

export const BookingEngineNoteModel: Model<BookingEngineNoteDoc> =
  mongoose.models.BookingEngineNote ??
  mongoose.model<BookingEngineNoteDoc>("BookingEngineNote", bookingEngineNoteSchema);

// ── Mapper ────────────────────────────────────────────────────────────────────

export function toBookingEngineNote(doc: BookingEngineNoteDoc): BookingEngineNote {
  return {
